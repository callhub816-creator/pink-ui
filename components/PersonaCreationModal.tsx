  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in">
      ...
      ">
        ...
        <div className={`relative p-6 bg-gradient-to-r ${mode.gradientConfig} text-[#4A2040]`}>
          <button onClick={onClose} className="absolute top-4 right-4 p-2 bg-black/5 hover:bg-black/10 rounded-full transition-colors">
            <X size={20} className="text-[#4A2040]" />
          </button>
          <div className="space-y-1">
            <span className="text-[10px] uppercase tracking-widest font-bold opacity-70 flex items-center gap-1 text-[#4A2040]">
              <Sparkles size={10} className="text-[#4A2040]" /> PREMIUM ACTIVE
            </span>
            <h2 className="text-2xl font-serif-display font-medium text-[#4A2040]">{mode.title}</h2>
            <p className="text-[#4A2040]/80 text-sm font-light">{mode.subtitle}</p>
          </div>
        </div>
          ...
          <div className="p-12 flex flex-col items-center justify-center text-center space-y-6 min-h-[400px]">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[#5e3a58] text-sm font-medium animate-pulse">
                {isGeneratingPhoto ? "Generating high-quality AI Portrait..." : "Finalizing personality traits..."}
              </p>
            </div>
          </div>
        ) : (
              ...
              <div className="bg-red-50 border border-red-100 rounded-xl p-4 flex flex-col gap-2 animate-in slide-in-from-top-2">
                ...
                <p className="text-xs text-red-600/80">
                  {error === 'High Traffic (429)'
                    ? 'We are experiencing high traffic. Please try generating again.'
                    : 'Could not generate portrait. Please try again.'}
                </p>
              </div>
            )}
